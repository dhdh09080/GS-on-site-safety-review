        ...
        with m_tab:
            ...
            # ---------------------------------------------------------
                ...
                with st.expander("🛠️ 템플릿(점수표) 다시 불러오기 / 버전 변경"):
                    ...
                    sel_ver = st.selectbox("적용할 템플릿 버전 선택", all_versions, key="reload_ver")
                    if st.button("🔄 선택한 버전으로 템플릿 덮어쓰기", type="primary"):
                        t_df_reload = load_template(sel_ver).fillna("")
                        new_details = {}
                        for _, itm in t_df_reload.iterrows():
                            new_details[str(itm['id'])] = {
                                "score": None, "is_na": False, "max": int(itm['max_score']), "memo": "",
                                "category": str(itm['category']).strip(), "pdca": str(itm['pdca']).strip(),
                                "item_name": str(itm['item_name']).strip(), "penalty": str(itm['penalty']).strip()
                            }
                ...
                for i, cat in enumerate(current_cats):
                    with tabs[i]:
                        if cat == "항목 없음" or cat == "분류없음":
                            st.warning("⚠️ 이 심사 방은 점검 항목이 비어있습니다. 위쪽의 [🛠️ 템플릿 다시 불러오기]를 눌러 템플릿을 적용해주세요.")
                            continue
